...
// Shared constants/helpers are loaded from chess-config.js
// ─── Main Component ───────────────────────────────────────────────────────────
...
function Chess3D() {
  ...
  const sr = useRef({
    ...
    mode: null, // 'pvp' | 'pvai'
    playerColor: W,
    status: "idle", // idle | playing | check | checkmate | stalemate | draw | resigned | timeout
    timeControlId: "blitz",
    clockIncrementMs: 0,
  ...
  const doMove = useCallback((from, to, { relay = true, promotion = null } = {}) => {
    ...
    s.positionCounts.set(key, (s.positionCounts.get(key) ?? 0) + 1);
  ...
  const aiMove = useCallback(() => {
    ...
    s.aiTimerId = setTimeout(async () => {
      ...
      if (chosenMove) doMove(chosenMove.from, chosenMove.to, { promotion: chosenMove.promotionType ?? null });
    }, delay);
  ...
  useEffect(() => {
    ...
    // ── Coordinate labels — floating outside board edges (all 4 sides) ─────
    // Sprites always face camera, so labels remain readable as you orbit.
    ...
    // File letters on near and far edges.
    ...
    // Rank numbers on left and right edges.
    ...
    // Render loop — fade piece-symbol sprites and edge coordinate labels
    ...
    const animate = () => {
      ...
      // Floating edge labels fade gently as camera goes low to reduce clutter.
  ...
  const statusMsg = () => {
    ...
    if (ui.status === "draw") return `🤝 Draw — ${drawReasonLabel(ui.drawReason)}`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        display: "flex", alignItems: "center", gap: "12px", zIndex: 10, flexShrink: 0,
      }}>
        <span style={{ fontSize: "1.35em", color: "#d4a843", fontWeight: "bold", letterSpacing: "0.06em" }}>
          ♟ 3D CHESS
        </span>
        ...
        <span style={{ marginLeft: "auto", color: "#5d4f3a", fontSize: "0.74em" }}>
        ...
        {ui.mode && (
          <aside style={{
            ...
            <div style={{
              ...
              {turnSubline && (
                <div style={{ color: "#bda883", fontSize: "0.82em", letterSpacing: "0.02em" }}>
                  {turnSubline}
                </div>
              )}
            </div>
            ...
            {ui.mode === "net" && (
              <div style={{
                ...
                <div style={{
                  ...
                  {net.peerStatus === "connected" ? "CONNECTED" : "DISCONNECTED"}
                </div>
                <div style={{ color: "#bda883", fontSize: "0.77em", marginTop: "4px" }}>
                  {net.statusMsg || ""}
                </div>
              </div>
            )}
            ...
            {hasClock && (
              <div style={{
                ...
                <div style={{ color: "#8a7a58", fontSize: "0.7em", letterSpacing: "0.06em", marginBottom: "8px" }}>
                  CLOCK · {activeTimeControl.label}
                </div>
            ...
            {ui.mode === "pvai" && (
              <div style={{
                ...
                <div style={{ color: "#9bc2ff", fontSize: "0.74em", letterSpacing: "0.05em", marginBottom: "4px" }}>
                  AI SETTINGS
                </div>
                <div style={{ color: "#c9ddff", fontSize: "0.83em" }}>
                  {aiSummary}
                </div>
                {ui.aiThinking && (
                  <div style={{ color: "#a9c7ff", fontSize: "0.76em", marginTop: "6px" }}>AI is thinking...</div>
                )}
              </div>
            )}
            ...
            <div style={{
              ...
              <div style={{ color: "#8a7a58", fontSize: "0.7em", letterSpacing: "0.06em", marginBottom: "6px" }}>
                OPENING
              </div>
              <div style={{ color: "#d4c5a9", fontSize: "0.8em", lineHeight: 1.4 }}>
                <div>White: {ui.openingWhite}</div>
                <div>Black: {ui.openingBlack}</div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ overflowY: "auto", minHeight: 0, paddingRight: "4px" }}>
                {notationRows.length === 0 ? (
                  <div style={{ color: "#6a5a3a", fontSize: "0.8em", padding: "6px 0" }}>
                    Moves will appear here once the game starts.
                  </div>
                ) : (
                  notationRows.map((row, idx) => (
                    <div
                      key={row.ply}
                      style={{
                        display: "grid",
                    ...
                    >
                      <div style={{ color: "#8a7a58" }}>{row.ply}.</div>
                      <div style={{ color: "#f0d9b5" }}>{row.white || "-"}</div>
                      <div style={{ color: "#d4c5a9" }}>{row.black || ""}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div style={{ display: "grid", gap: "8px", gridTemplateColumns: "1fr 1fr", flexShrink: 0 }}>
              {ui.mode && !isOver && btn("🏳 Resign", () => resignGame(), "#6a1f1f", { width: "100%" })}
              {ui.mode && ui.mode !== "net" && btn("↺ Restart", () => startGame(ui.mode, ui.playerColor, ui.timeControlId), "#1a2a1a", { width: "100%" })}
              {ui.mode && btn("⟵ Menu", () => {
                disconnectNet();
                setPresenceStatus("lobby");
      ...
      {ui.mode && ui.promotion && (
        <div style={overlayStyle}>
          <div style={{ ...cardStyle, minWidth: "420px", maxWidth: "460px" }}>
            <h2 style={{ margin: "0 0 10px", color: "#d4a843", fontSize: "1.55em" }}>Choose Promotion</h2>
            <p style={{ color: "#7d6e4f", margin: "0 0 16px", fontSize: "0.82em" }}>
              Select the piece your pawn becomes.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(140px, 1fr))", gap: "10px" }}>
              {promotionOptions.map((opt) => (
                ...
                >
                  <span style={{ fontSize: "1.3em" }}>{ui.promotion.color === W ? opt.white : opt.black}</span>
                  {opt.label}
                </button>
              ))}
            </div>
